...
export const BlogPostPage: NextPage<{
  ...
  return (
    <Layout>
      ...
      <InfoBlock>
        <InfoBlockTitle className="text-gradient-blue-purple pb-2">
          Related Posts
        </InfoBlockTitle>
        <div className="w-full grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          {additionalBlogPosts.map((b, i) => (
            <BlogCard
              key={b.slug}
              blog={b}
              color={getHoverColorFromNumber(i)}
            />
          ))}
        </div>
      </InfoBlock>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  return {
    props: {
      blogPost,
      markdown,
      additionalBlogPosts: recentBlogPosts.filter(
        (i) => i.slug !== blogPost.slug
      ),
    },
  };
};
